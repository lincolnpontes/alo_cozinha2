(function (global) {
    ...
    const SEEDED_KEY = 'alo_demo_seeded_v1';
    const PREFIX = 'alo_demo:';
    ...
    function seed() {
        ...
        const today = dateKey();
        const areas = [
            { id: 'panelas', nome: 'Panelas', emoji: '🥘', tipo: 'envio', ativo: true, modulos: { kds: true, checklist: false } },
        ...
        const mainDatabase = {
            ...
            tarefas: [
                ...
                { id: 'demo_task_salao', nome: 'Organizar mesas do salão', setorId: 'salao', funcionarioId: 'demo_carlos', ativo: true, prioridade: 'normal', tempoEsperadoMin: 15, instrucoes: 'Limpar mesas, alinhar cadeiras e conferir os itens.', programacoes: [{ id: 'abertura', horario: '09:00', recorrencia: 'diaria', dias: [0,1,2,3,4,5,6], dataInicio: today, alarme: false }] }
            ],
        ...
        const compras = {
            ...
            colaboradores: [{ id: 'demo_ana', nome: 'Ana', emoji: '👩‍🍳', ativo: true, isAdmin: true, catsPermitidasPedido: ['cat_secos','cat_hortifruti'], catsPermitidasCompras: ['cat_secos','cat_hortifruti'] }],
            produtos: [
                { id: 'prod_arroz', nome: 'Arroz parboilizado', categoria: 'cat_secos', unidades: ['kg'], fornecedores: ['forn_central'], ativo: true, historicoPrecos: [{ id: 'preco_arroz', data: today, preco: 6.49, unidade: 'kg', fornecedorId: 'forn_central', registradoEm: now, atualizadoEm: now }] },
                { id: 'prod_tomate', nome: 'Tomate', categoria: 'cat_hortifruti', unidades: ['kg'], fornecedores: ['forn_central'], ativo: true, historicoPrecos: [{ id: 'preco_tomate', data: today, preco: 7.9, unidade: 'kg', fornecedorId: 'forn_central', registradoEm: now, atualizadoEm: now }] },
                { id: 'prod_leite', nome: 'Leite condensado', categoria: 'cat_secos', unidades: ['un'], fornecedores: ['forn_central'], ativo: true, historicoPrecos: [{ id: 'preco_leite', data: today, preco: 6.2, unidade: 'un', fornecedorId: 'forn_central', registradoEm: now, atualizadoEm: now }] }
            ],
            pedidosAtivos: [
                { idUnico: `demo_${now}_arroz`, produtoId: 'prod_arroz', quantidade: 10, unidade: 'kg', status: 'rascunho', dataStatus: now },
                { idUnico: `demo_${now}_tomate`, produtoId: 'prod_tomate', quantidade: 4, unidade: 'kg', status: 'enviado', dataEnvio: now - 3600000, dataStatus: now - 3600000 }
            ],
        ...
        const documents = {
            revision: 0, outbox: [], documents: [{ id: 'demo_doc_alvara', templateId: 'alvara-sanitario', nome: 'Alvará sanitário', categoria: 'Licenças', orgao: 'Vigilância Sanitária', numero: '2026/001', emitidoEm: today, venceEm: `${new Date().getFullYear() + 1}-12-31`, observacoes: 'Documento de demonstração.', arquivo: null, atualizadoEm: now, revisao: 1 }]
        };
        const labels = {
            produtos: [
