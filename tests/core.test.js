...
function testV2027TaskExperience() {
    ...
    assert.equal(html.includes('<div class="module-home-version">v2.1.47</div>'), true, 'a tela inicial deve mostrar a versão');
...
function testComprasUsesUnifiedHost() {
    ...
    const frame = {
        dataset: { src: 'modules/compras/index.html?v=2.1.47' },
        getAttribute() { return ''; },
        setAttribute() {},
        addEventListener() {}
    };
...
function testV217ModuleArchitecture() {
    ...
    assert.equal(contracts.appVersion, '2.1.47');
    ...
    assert.equal(html.includes('core/module-host.js?v=2.1.47'), true);
    assert.equal(html.includes('core/shared-data.js?v=2.1.47'), true);
    assert.equal(html.includes('modules/compras/index.html?embedded=1&amp;v=2.1.47'), true);
    assert.equal(html.includes('modules/l42/index.html?embedded=1&amp;v=2.1.47'), true);
    assert.equal(html.includes('modules/l42/icon.png?v=2.1.47'), true);
    assert.equal(shellCache.includes('./modules/checklist/app.js?v=2.1.47'), true);
    assert.equal(shellCache.includes('./core/shared-data.js?v=2.1.47'), true);
...
function testV219UnifiedPeopleAndDataHub() {
    ...
    assert.equal(shellCache.includes('./modules/l42/cloud.js?v=2.1.47'), true, 'a sincronização de Etiquetas deve funcionar offline após o primeiro carregamento');
}
...
function testV2110AccessSyncAndApkPolish() {
    ...
    assert.equal(l42.includes('assets/qr-reader.svg?v=2.1.47'), true);
    assert.equal(l42.includes('assets/printer-controls.svg?v=2.1.47'), true);
    ...
    assert.equal(shellCache.includes('./modules/l42/assets/qr-reader.svg?v=2.1.47'), true);
    assert.equal(shellCache.includes('./modules/l42/assets/printer-controls.svg?v=2.1.47'), true);
    ...
    assert.equal(kds.includes('window.AloNative.saveDocumentBase64'), true, 'o backup completo deve usar o salvamento nativo no APK');
    ...
    assert.equal(androidBuild.includes('versionCode 77'), true);
    assert.equal(androidBuild.includes('versionName "2.1.47"'), true);
    assert.equal(apkScript.includes('Alo-Cozinha-v2.1.47.apk'), true);
}

function testV2141SupabaseCloud() {
    const html = fs.readFileSync(path.join(root, 'index.html'), 'utf8');
    const cloud = fs.readFileSync(path.join(root, 'core', 'cloud.js'), 'utf8');
    const api = fs.readFileSync(path.join(root, 'core', 'api.js'), 'utf8');
    const edge = fs.readFileSync(path.join(root, 'supabase', 'functions', 'alo-cozinha-sync', 'index.ts'), 'utf8');
    const migration = fs.readFileSync(path.join(root, 'supabase', 'migrations', '202609010001_unified_cloud.sql'), 'utf8');
    const androidBuild = fs.readFileSync(path.join(root, 'android', 'app', 'build.gradle'), 'utf8');
    const shellCache = fs.readFileSync(path.join(root, 'service-worker.js'), 'utf8');

    assert.equal(fs.existsSync(path.join(root, 'google-apps-script.gs')), false, 'o backend GAS não deve acompanhar a versão Supabase');
    assert.equal(html.includes('id="configUrlApp"'), false, 'o usuário não deve configurar URL de backend');
    assert.equal(html.includes('id="cloudAccountDisconnected"'), true, 'a conta deve ser conectada somente nas configurações');
    assert.equal(cloud.includes("const ENDPOINT = `${SUPABASE_URL}/functions/v1/alo-cozinha-sync`"), true);
    assert.equal(cloud.includes('candidate.pathname.replace'), true, 'ações com query string devem receber autenticação');
    assert.equal(api.includes('global.AloCloud.fetch(url, options)'), true);
    assert.equal(edge.includes('ctx.userClient.schema("api").rpc("sync_module_state"'), true);
    ...
    assert.equal(shellCache.includes('./vendor/supabase.min.js?v=2.1.47'), true);
}
...
(async () => {
    ...
    console.log('Testes críticos da v2.1.47 passaram.');
})().catch(error => {
    console.error(error);
    process.exitCode = 1;
});
