...
function testV2027TaskExperience() {
    ...
    assert.equal(html.includes('<div class="module-home-version">v2.1.50</div>'), true, 'a tela inicial deve mostrar a versão');
...
function testComprasUsesUnifiedHost() {
    ...
    const frame = {
        dataset: { src: 'modules/compras/index.html?v=2.1.50' },
        getAttribute() { return ''; },
        setAttribute() {},
        addEventListener() {}
    };
...
function testV217ModuleArchitecture() {
    ...
    assert.equal(contracts.appVersion, '2.1.50');
    ...
    assert.equal(html.includes('core/module-host.js?v=2.1.50'), true);
    assert.equal(html.includes('core/shared-data.js?v=2.1.50'), true);
    assert.equal(html.includes('modules/compras/index.html?embedded=1&amp;v=2.1.50'), true);
    assert.equal(html.includes('modules/l42/index.html?embedded=1&amp;v=2.1.50'), true);
    assert.equal(html.includes('modules/l42/icon.png?v=2.1.50'), true);
    assert.equal(shellCache.includes('./modules/checklist/app.js?v=2.1.50'), true);
    assert.equal(shellCache.includes('./core/shared-data.js?v=2.1.50'), true);
...
function testV219UnifiedPeopleAndDataHub() {
    ...
    assert.equal(shellCache.includes('./modules/l42/cloud.js?v=2.1.50'), true, 'a sincronização de Etiquetas deve funcionar offline após o primeiro carregamento');
}
...
function testV2110AccessSyncAndApkPolish() {
    ...
    assert.equal(l42.includes('assets/qr-reader.svg?v=2.1.50'), true);
    assert.equal(l42.includes('assets/printer-controls.svg?v=2.1.50'), true);
    ...
    assert.equal(shellCache.includes('./modules/l42/assets/qr-reader.svg?v=2.1.50'), true);
    assert.equal(shellCache.includes('./modules/l42/assets/printer-controls.svg?v=2.1.50'), true);
    ...
    assert.equal(androidBuild.includes('versionCode 80'), true);
    assert.equal(androidBuild.includes('versionName "2.1.50"'), true);
    assert.equal(apkScript.includes('Alo-Cozinha-v2.1.50.apk'), true);
}
...
function testV2141SupabaseCloud() {
    ...
    assert.equal(shellCache.includes('./vendor/supabase.min.js?v=2.1.50'), true);
}
...
(async () => {
    ...
    console.log('Testes críticos da v2.1.50 passaram.');
})().catch(error => {
    console.error(error);
    process.exitCode = 1;
});
